 ...
 */
contract TimelockController is AccessControl, IERC721Receiver, IERC1155Receiver, AutomationCompatibleInterface {
    //Types
    struct passedProposalsType {
        address target;
        uint256 value;
        bytes payload;
        bytes32 predecessor;
    ...
    function _schedule(bytes32 id, uint256 delay) private {
        ...
        listOfIds.push(id); //storing ids of proposals in a varialble
    ...
    /**
     * @dev chainlink integration with DAOs for automatic execution of passed proposals after minDelay.
     * @dev implementing the chainlink automation
     */
    function checkUpkeep(
        bytes memory /* *checkData*/
    ) public override returns (bool upkeepNeeded, bytes memory performData) {
        for (uint256 i = 0; i < listOfIds.length; ++i) {
            if (isOperationReady(listOfIds[i])) {
                performData = abi.encode(listOfIds[i]);
                upkeepNeeded = true;
                break;
            }
        }
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        require(upkeepNeeded, "TimelockController: operation is not ready");
